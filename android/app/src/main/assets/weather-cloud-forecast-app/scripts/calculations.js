export const DAY_BACKGROUND = 'https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=1950&q=80';
export const NIGHT_BACKGROUND = 'https://images.unsplash.com/photo-1503264116251-35a269479413?auto=format&fit=crop&w=1950&q=80';
...
export function dewPointSpread(temperature, dewPoint) {
...
export function buildObservationGuidance({
  analysis,
  currentElevation,
  sunriseTime,
  sunsetTime,
...
export function analyzeCurrentCloudSea(current, elevation) {
...
export function getCurrentCloudCover(current) {
  return Number(current?.cloud_cover ?? current?.cloudcover ?? 0);
}

export function getCurrentLowCloudCover(current) {
  return Number(current?.cloud_cover_low ?? current?.cloudcover_low ?? 0);
}

export function getHourlyCloudCover(hourly, start, count = 24) {
  const values = hourly?.cloud_cover ?? hourly?.cloudcover ?? [];
  return values.slice(start, start + count).map((value) => Number(value ?? 0));
}

export function getHourlyLowCloudCover(hourly, start, count = 24) {
  const values = hourly?.cloud_cover_low ?? hourly?.cloudcover_low ?? [];
  return values.slice(start, start + count).map((value) => Number(value ?? 0));
}

export function minOrZero(values) {
  return values.length ? Math.min(...values) : 0;
}

export function maxOrZero(values) {
  return values.length ? Math.max(...values) : 0;
}

export function analyzeDayCloudSea(hourly, start, elevation) {
...
export function pickBackgroundImage(timeString) {
  const hour = new Date(timeString).getHours();
